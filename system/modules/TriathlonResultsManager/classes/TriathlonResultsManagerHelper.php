...
class TriathlonResultsManagerHelper
{
	...
	/**
	 * Extract the distances with disciplines to a plain value array.
	 *
	 * @param array  $arrDistances      The complex array of distance data.
	 ...
	 */
	public static function getPlainDistances($arrDistances, $blnAddDiscipline)
	{
		if (is_array($arrDistances))
		{
			$arrPlainDistances = array();
			foreach ($arrDistances as $distance)
			{
				$strDistance = sprintf($GLOBALS['TL_LANG']['TriathlonResultsManager']['distance_format'], str_replace('.', $GLOBALS['TL_LANG']['MSC']['decimalSeparator'], $distance['distance']['value']), $distance['distance']['unit']);
				
				if ($blnAddDiscipline)
				{
					$strDiscipline = "";
					if ($distance['discipline'] == 'others' && !empty($distance['discipline_freetext']))
					{
						$strDiscipline = $distance['discipline_freetext'];
					}
					else
					{
						$strDiscipline = $GLOBALS['TL_LANG']['TriathlonResultsManager']['disciplines'][$distance['discipline']];
					}
					
					$arrPlainDistances[] = sprintf($GLOBALS['TL_LANG']['TriathlonResultsManager']['discipline_distance_format'], $strDiscipline, $strDistance);
				}
				else
				{
					$arrPlainDistances[] = $strDistance;
				}
			}
			return $arrPlainDistances;
		}
		return null;
	}
