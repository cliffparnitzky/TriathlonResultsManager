...
class ContentTriathlonResultsManagerResults extends \ContentElement
{
						...
						if ($objResults != null)
						{
							while ($objResults->next())
							{
								...
								if ($objResultsCompetitions->performanceEvaluation == 'distance')
								{
									...
									$arrResult['formattedDistance'] = sprintf($GLOBALS['TL_LANG']['TriathlonResultsManager']['distance_format'], \TriathlonResultsManagerHelper::addGroupedThousands($arrDistance['value']), $arrDistance['unit']);
								}
								else
								{
	...
	private function getKeyForRatingType($strType)
	{
		...
		if (empty($arrOrder))
		{
			$arrOrder = $GLOBALS['TL_DCA']['tl_content']['fields']['triathlonResultsManagerSortResultRatingTypeOrder']['options'];
		}
		...
		if ($intIndex === FALSE)
		{
			$intIndex = count($GLOBALS['TL_DCA']['tl_content']['fields']['triathlonResultsManagerSortResultRatingTypeOrder']['options']);
		}
		return $intIndex . "_";
	}
}
